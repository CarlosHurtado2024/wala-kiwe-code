...
export default function NuevaFamiliaGuiadaPage() {
    ...
    const handleSave = () => {
        ...
        startTransition(async () => {
            ...
            // Default to the first member as representative names (though action now uses surnames)
            const representativeName = `${selectedMembers[0].primer_nombre} ${selectedMembers[0].primer_apellido}`;
            const vereda = selectedMembers[0].direccion_actual;
            const jefeId = selectedMembers[0].id; // We assume the first selected is the head for now

            const result = await createFamiliaAction(memberIds, representativeName, vereda, jefeId);
    ...
    useEffect(() => {
        const fetchResults = async () => {
            ...
            const { data, error } = await supabase
                .from('comuneros')
                .select('*')
                .or(`primer_nombre.ilike.%${searchTerm}%,primer_apellido.ilike.%${searchTerm}%,numero_documento.ilike.%${searchTerm}%`)
                .limit(10);

            if (!error && data) {
                setResults(data as any as Comunero[]);
            }
    ...
    return (
        <div className="flex-1 flex flex-col min-h-0 bg-background">
            {/* Header */}
            <header className="h-16 shrink-0 border-b border-border flex items-center justify-between px-6 lg:px-8 bg-card/80 backdrop-blur-sm sticky top-0 z-50">
                <div className="flex items-center gap-3">
                    <div className="bg-primary/10 p-2 rounded-xl border border-primary/20">
                        <Search className="w-5 h-5 text-primary" />
                    </div>
                    <div>
                        <h2 className="text-lg font-bold text-foreground leading-tight">Crear Familia</h2>
                        <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider">Busca y agrupa integrantes</p>
                    </div>
                </div>
            </header>
                            ...
                            <div className="flex flex-wrap gap-2 min-h-[40px] items-center">
                                ...
                                {selectedMembers.map((member) => (
                                    <div key={member.id} className="flex items-center gap-1.5 bg-primary/10 border border-primary/20 rounded-full pl-3 pr-1 py-1 shadow-sm">
                                        <span className="text-xs font-bold text-primary">{member.primer_nombre} {member.primer_apellido}</span>
                                        <button
                                                ...
                                                <div key={r.id} className={`flex items-center justify-between p-4 transition-colors cursor-pointer group ${isSelected ? 'bg-primary/10' : 'hover:bg-secondary/30'}`}>
                                                    <div className="flex items-center gap-4">
                                                        <div className={`w-12 h-12 rounded-full overflow-hidden shrink-0 flex items-center justify-center text-lg font-bold uppercase
                                                            ...
                                                            `}>
                                                            {r.primer_nombre.charAt(0)}{r.primer_apellido.charAt(0)}
                                                        </div>
                                                        <div>
                                                            <p className="text-base font-bold text-foreground">{r.primer_nombre} {r.primer_apellido}</p>
                                                            <p className="text-sm text-muted-foreground">Doc: {r.numero_documento || 'Sin ID'} • {r.direccion_actual || 'Dirección no reportada'}</p>
                                                        </div>
                                                    </div>
